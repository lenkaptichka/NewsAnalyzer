...
module.exports = {
    ...
    output: {
        ...
        filename: './js/[name].[chunkhash].js' // Имя файла, куда вебпак положит код
    },
    module: {
        rules: [{
    ...
    plugins: [
        ...
        new HtmlWebpackPlugin ({
            inject: false, // Cтили НЕ нужно прописывать внутри тегов
            template: './src/pages/about/about.html', // Откуда брать образец для сравнения с текущим видом проекта
            filename: 'about.html' // Имя выходного файла, то есть того, что окажется в папке dist после 
        }),
        new HtmlWebpackPlugin ({
            inject: false, // Cтили НЕ нужно прописывать внутри тегов
            template: './src/pages/analytics/analytics.html', // Откуда брать образец для сравнения с текущим видом проекта
